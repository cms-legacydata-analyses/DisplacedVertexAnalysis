// -*- C++ -*-
//
// Package:    SimuMuonAnalyzer2
// Class:      SimuMuonAnalyzer2
// 
/**\class SimuMuonAnalyzer2 SimuMuonAnalyzer2.cc Ntuple/SimuMuonAnalyzer2/src/SimuMuonAnalyzer2.cc
...
class SimuMuonAnalyzer2 : public edm::EDAnalyzer {
   public:
      explicit SimuMuonAnalyzer2(const edm::ParameterSet&);
      ~SimuMuonAnalyzer2();
...
//
SimuMuonAnalyzer2::SimuMuonAnalyzer2(const edm::ParameterSet& iConfig)
:
...
SimuMuonAnalyzer2::~SimuMuonAnalyzer2()
{
...
void
SimuMuonAnalyzer2::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   using namespace edm;
...
bool
SimuMuonAnalyzer2::cmsStandardCuts(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
 bool ret = false;
 int disp = 0;
 int numHighPurity = 0;
...
bool 
SimuMuonAnalyzer2::matchingCuts( bool purity, double pt, int hits, int hits3D, double eta, double dxy, double dxyError)
{
	bool ret = false;
...
double 
SimuMuonAnalyzer2::deltaR(double obj1Phi, double obj1Eta, double obj2Phi, double obj2Eta)
{
...
double 
SimuMuonAnalyzer2::conePt(int forbiddenIndex1, int forbiddenIndex2, double eta, double phi, int numTracks,const edm::Event& iEvent, const edm::EventSetup& iSetup )
{  
	using namespace edm;
	using namespace reco;
    Handle<TrackCollection> tracks;
...
double 
SimuMuonAnalyzer2::mCos(double phi1, double eta1, double phi2, double eta2 )
{double cosAlpha = cos(eta1)*cos(phi1)*cos(eta2)*cos(phi2) + cos(eta1)*sin(phi1)*cos(eta2)*sin(phi2) + sin(eta1)*cos(eta2);
	
	return cosAlpha;
}
double 
SimuMuonAnalyzer2::mTheta(double ax, double ay, double bx, double by)
{
...
double 
SimuMuonAnalyzer2::invMass(double px1, double py1, double pz1, double px2 , double py2,  double pz2)
{
...
double
SimuMuonAnalyzer2::dotProduct(double x1, double y1 , double x2, double y2)
{
	double ret;
	ret = x1*x2+y1*y2;
	return ret;
...
bool
SimuMuonAnalyzer2::impactParameterCut(reco::TrackCollection::const_iterator it1, reco::TrackCollection::const_iterator it2, reco::BeamSpot beamSpot)
{    
...
void 
SimuMuonAnalyzer2::beginRun(edm::Run const&, edm::EventSetup const&)
{
}

// ------------ method called when ending the processing of a run  ------------
void 
SimuMuonAnalyzer2::endRun(edm::Run const&, edm::EventSetup const&)
{
}

// ------------ method called when starting to processes a luminosity block  ------------
void 
SimuMuonAnalyzer2::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}

// ------------ method called when ending the processing of a luminosity block  ------------
void 
SimuMuonAnalyzer2::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void
SimuMuonAnalyzer2::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(SimuMuonAnalyzer2);
